...
- When the user is directed to the request flow but has hit the limit of allowed requests: show message “Please call to schedule”
- The user selects a facility that supports neither direct scheduling nor requests: show message “Please call to schedule”
- User has never had an appointment for selected care type at facility: show message, direct to call
- The user selects “current location” from drop down and has blocked browser from using location: display alert
...
- Appointment time available: display calendar with appointment slots of the selected provider
- API call to fetch slots fails, and user has requests enabled: Display alert with request option
- API call to fetch slots fails, and user does not have requests enabled: Display alert with call to schedule
...
Would you like me to also convert this into a proper clean markdown structure with headings, bullets, and maybe a table of contents?
